...
class PaymentController extends Controller
{
    ...
    public function print($id)
    {
        try {
            ...
            Log::info('Dati del pagamento:', [
                'id' => $payment->id,
                'student_name' => $payment->student->name ?? 'N/A',
                'student_surname' => $payment->student->surname ?? 'N/A',
                'birth_date' => $payment->student->birth_date ?? 'N/A',
                'course' => $payment->course->name ?? 'N/A',
                'payment_tpe' => $payment->payment_type->name ?? 'N/A',
                'month' => $payment->month->name ?? 'N/A',
                'year' => $payment->year ?? '-',
                'amount' => $payment->amount,
            ]);
            $pdf = Pdf::loadView('admin.pdf.payment', compact('payment'));
            //$pdf = PDF::loadView('admin.course.pdf.show', compact('course'));

               return $pdf->download('Pagamento_' . $payment->id . '.pdf');
        } catch (Exception $e) {

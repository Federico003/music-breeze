                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            @php
                                ...
                                $teacherId = auth()->id(); // o come recuperi l'id insegnante loggato
$totalStudents = DB::table('course_enrollments')
    ...
    ->count('student_id');
                            ...
                            <h3 class="text-2xl font-bold text-orange-600 dark:text-indigo-600">{{ $totalStudents }}</h3>
                        </div>
                        <div
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            @php
                                ...
                                $lessonsToday = DB::table('lessons')
                                    ->join(
                                        'course_enrollments',
                                        'lessons.course_enrollment_id',
                                        '=',
                                        'course_enrollments.id',
                                    )
                                    ->where('course_enrollments.teacher_id', $teacherId)
                                    ->whereDate('lessons.day', now()->toDateString())
                                    ->count();
                            @endphp

                            <p class="text-black-500 dark:text-gray-400">LEZIONI OGGI</p>
                            <h3 class="text-2xl font-bold text-orange-600 dark:text-indigo-600">{{ $lessonsToday }}</h3>
                        </div>
                        <div
                            class="w-12 h-12 rounded-full bg-orange-100 dark:bg-indigo-600 flex items-center justify-center text-orange-600 dark:text-indigo-200">
                            <svg class="w-8 h-8" xmlns="http://www.w3.org/2000/svg"

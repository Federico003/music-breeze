    ...
    <div class=" py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <section class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 m-4 card-hover transition duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        @php
                            $count = DB::select('SELECT COUNT(*) AS total FROM `music-breeze`.students');
                        @endphp

                        <h3 class="text-2xl font-bold text-orange-600 dark:text-indigo-600">{{ $count[0]->total }}</h3>
                    </div>
                    <div
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 m-4 card-hover transition duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        @php
                            $count = DB::select('SELECT COUNT(*) AS total FROM `music-breeze`.teachers');
                        @endphp
                        <h3 class="text-2xl font-bold text-orange-600 dark:text-indigo-600">{{ $count[0]->total }}</h3>
                    </div>
                    <div
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 m-4 card-hover transition duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        @php
                            $todayLessons = DB::select('SELECT COUNT(*) AS total FROM lessons WHERE day = CURDATE()');
                        @endphp
                        <h3 class="text-2xl font-bold text-orange-600 dark:text-indigo-600">
                            {{ $todayLessons[0]->total }}</h3>
                    </div>
                    <div
                        class="w-12 h-12 rounded-full bg-orange-100 dark:bg-indigo-600 flex items-center justify-center text-orange-600 dark:text-indigo-200">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 m-4 card-hover transition duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-black-500 dark:text-gray-400">Pagamenti {{ date('Y') }}</p>

                        @php
                            $totalPayments = DB::select(
                                'SELECT SUM(amount) AS total FROM payment WHERE year = YEAR(CURDATE())',
                            );
                        @endphp

                        <h3 class="text-2xl font-bold text-orange-600 dark:text-indigo-600">
                            €{{ number_format($totalPayments[0]->total ?? 0, 2, ',', '.') }}</h3>
                    </div>
                    <div
                        class="w-12 h-12 rounded-full bg-orange-100 dark:bg-indigo-600 flex items-center justify-center text-orange-600 dark:text-indigo-200">
                        <svg class="w-8 h-8" xmlns="http://www.w3.org/2000/svg"
        ...
        <section class="mb-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 ">
                ...
                <button onclick="window.location.href='{{ route('admin.create-course') }}'"
                    ...
                    <span class="text-gray-700 dark:text-gray-400">Nuovo Corso</span>
                </button>
                ...
                <button onclick="window.location.href='{{ route('admin.show-lessons') }}'"
                    class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 m-4 flex flex-col items-center justify-center hover:bg-orange-200 dark:hover:bg-indigo-700 transition duration-300">
                    <div
                        class="w-10 h-10 rounded-full bg-orange-100 dark:bg-indigo-600 flex items-center justify-center text-orange-600 dark:text-indigo-200 mb-2">
                        <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg"

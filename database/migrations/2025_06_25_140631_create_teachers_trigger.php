...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::unprepared('
            CREATE TRIGGER trg_after_insert_user_teacher
            AFTER INSERT ON users
            FOR EACH ROW
            BEGIN
                IF (SELECT name FROM user_types WHERE id = NEW.user_type_id) = "insegnante" THEN
        ...
        DB::unprepared('
            CREATE TRIGGER trg_after_update_user_teacher
            ...
            BEGIN
                ...
                IF new_type = "insegnante" THEN
                    IF old_type != "insegnante" THEN
                        INSERT INTO teachers (
                            id, name, surname, birth_date, birth_place, gender, address, city, country,
                            postal_code, phone, email, password, created_at, updated_at
                        ) VALUES (
        ...
        DB::unprepared('
            CREATE TRIGGER trg_after_delete_user_teacher
            AFTER DELETE ON users
            FOR EACH ROW
            BEGIN
                IF (SELECT name FROM user_types WHERE id = OLD.user_type_id) = "insegnante" THEN
                    DELETE FROM teachers WHERE id = OLD.id;
                END IF;
            END
        ');
    ...
    {
        DB::unprepared('DROP TRIGGER IF EXISTS trg_after_insert_user_teacher');
        DB::unprepared('DROP TRIGGER IF EXISTS trg_after_update_user_teacher');
        DB::unprepared('DROP TRIGGER IF EXISTS trg_after_delete_user_teacher');
    }
};
